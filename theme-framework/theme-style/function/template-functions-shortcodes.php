...
function good_food_slider_post_heading($cmsmasters_id, $type = 'post', $tag = 'h1', $link_redirect = false, $link_url = false, $show = true, $link_target = false) { 
	if ($type == 'post') {
...
function good_food_slider_post_exc_cont($type = 'post', $show = true) {
	if ($type == 'post') {
...
function good_food_slider_post_check_exc_cont($type = 'post') {
	$exc = good_food_slider_post_exc_cont($type, false);
...
function good_food_get_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
...
function good_food_get_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
...
function good_food_get_slider_post_category($cmsmasters_id, $taxonomy, $type = 'post', $show = true) {
	$out = '';
...
function good_food_slider_post_like($type = 'post', $show = true) {
	$out = '';
...
function good_food_slider_post_view($show = true) {
	$out = cmsmastersView();
...
function good_food_get_slider_post_comments($type = 'post', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($type == 'post') {
			$out = good_food_get_comments('cmsmasters_slider_post_comments');
		} elseif ($type == 'project') {
			$out = good_food_get_comments('cmsmasters_slider_project_comments');
		}
	}
...
function good_food_slider_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_attr__('Read More', 'good-food');
	}
	
	
	$out = '<a class="cmsmasters_slider_post_read_more cmsmasters_theme_icon_slide_next" href="' . esc_url(get_permalink($cmsmasters_id)) . '">' . esc_html($cmsmasters_post_read_more) . '</a>';
