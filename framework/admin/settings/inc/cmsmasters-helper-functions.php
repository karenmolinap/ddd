...
function good_food_get_admin_page() {
	global $pagenow;
...
function good_food_default_tab() {
	$current_page = good_food_get_admin_page();
...
function good_food_get_the_tab() {
	global $pagenow;
	
	$default_tab = good_food_default_tab();
...
function good_food_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'good-food') . '</a>';
		}
