...
if (!function_exists('good_food_load_theme_textdomain')) {
	function good_food_load_theme_textdomain() {
		load_theme_textdomain('good-food', CMSMASTERS_VARS . '/languages');
	}
}
...
if (!function_exists('good_food_plugin_activation')) {
	function good_food_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'good_food_plugin_activation', 10, 2);
...
function good_food_run_reinit_import_options($post_id, $key, $value) {
...
add_action('import_post_meta', 'good_food_run_reinit_import_options', 10, 3);
