	...
	<div id="comments" class="post_comments cmsmasters_woo_comments">
		<h3 class="post_comments_title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
				...
				printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'good-food' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
			} else {
			...
			<div id="review_form">
				<?php
					...
					if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
						$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( esc_html__( 'You must be <a href="%s">logged in</a> to post a review.', 'good-food' ), esc_url( $account_page_url ) ) . '</p>';
					}
